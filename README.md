...
## Files & structure
- `app.py` — FastAPI server and `/ask` endpoint
- `inserting_file.py` — loaders for PDFs and `.txt` files
- `jac_functions.py` — chunking, embedding, insertion, retrieval, and prompt construction
- `insert_chroma.py` — example ingestion script
- `scripts/generate_publication.py` — generates `publication.docx`
- `docs/diagram.svg` / `docs/diagram.png` — architecture diagrams
- `research_db/` — persistent ChromaDB files (not tracked by default)

---

## Reproducible artifacts
- `publication.docx` — short project publication generated by `scripts/generate_publication.py`.
- To regenerate:
...
Contributions are welcome — please open an issue or a PR. Consider adding:
